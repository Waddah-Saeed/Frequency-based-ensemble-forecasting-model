...
forecasts_eur <- DailywithOPERA(training1, validation, training2, outofsample, fh)

res_eur <- outofsample - forecasts_eur
ggAcf(res_eur) + ggtitle("ACF of residuals")
Box.test(res_eur,lag=10, fitdf=0, type="Lj")
...
forecasts_bitcoin <- DailywithOPERA(Close_1year_training, validation, Close_1year_insample, out_of_sample, fh)

res <- out_of_sample - forecasts_bitcoin
